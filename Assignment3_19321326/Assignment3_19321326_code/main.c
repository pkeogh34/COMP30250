...
void performCalculations(int choice) {
    ...
    sprintf(filename, "data_q%d.csv", choice);
    fp = fopen(filename, "w");
    ...
    for (int n = 100; n <= 2000; n += 100) {
        ...
        if (choice == 1) {
            ...
            double serialTime = serial(n, A, B, C);
            fprintf(fp, "%d, %f, %f, %f\n", n, serialTime, parallelTime, serialTime / parallelTime);
        }
...
// enter 1 or 2 for respective question number
